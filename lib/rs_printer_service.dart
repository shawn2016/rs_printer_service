...
class RSPrinterService {
  ...
  static Future<RSPrinterInterface> getPrinter(PrinterType type) async {
    if (_printer != null) {
      return _printer!;
    }
  ...
  static Future<bool> printTemplate(
    ...
    try {
      // 1. 解析模板
      final elements = TemplateParser.parse(templateXml, data);

      // 2. 获取打印机实例
      final printer = await getPrinter(type);
